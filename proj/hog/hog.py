...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if who:
            ...
            score1 += take_turn(num_1, score0, dice)
            if extra_turn(score1, score0) == False:
                who = other(who)
        else:
            num_0 = strategy0(score0, score1)
            score0 += take_turn(num_0, score1, dice)
    ...
    def say(score0, score1):
        if who == 0:
            if score0 - last_score > running_high:
                print(str(score0 - last_score) + " point(s)! The most yet for Player " + str(who))
            return announce_highest(who, score0, max(running_high, score0 - last_score))
        else:
            if score1 - last_score > running_high:
                print(str(score1 - last_score) + " point(s)! The most yet for Player " + str(who))
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    pos = 1
    for i in range(1, 11):
        tmp = make_averaged(roll_dice)(i, dice)
        if tmp > maxx:
            maxx, pos = tmp, i
    return pos
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    tmp_score = score + take_turn(0, opponent_score)
    if extra_turn(tmp_score, opponent_score):
        return 0
    else:
