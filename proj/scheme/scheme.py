...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        map_expr = expr.map(lambda x: scheme_eval(x, env))
        operator = map_expr.first
        validate_procedure(operator)
        return scheme_apply(operator, map_expr.rest, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    return cur # replace this with lines of your own code
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        "*** YOUR CODE HERE ***"
        child = Frame(self)
        tmpfs, tmpvs = formals, vals
        while tmpfs != nil:
            formal, val = tmpfs.first, tmpvs.first
            child.define(formal, val)
            tmpfs, tmpvs = tmpfs.rest, tmpvs.rest
        return child
        # END PROBLEM 10
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        fname, formals, body = target.first, target.rest, expressions.rest
...
def do_and_form(expressions, env):
    ...
    i = True
    while cur != nil:
        i = scheme_eval(cur.first, env)
        if is_false_primitive(i):
            return i
        cur = cur.rest
    return i
    # END PROBLEM 12
