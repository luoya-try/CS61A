...
def choose(paragraphs, select, k):
    ...
    for i in paragraphs:
        if select(i):
            tmp.append(i)
    return '' if k >= len(tmp) else tmp[k]
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i  in valid_words:
        if user_word == i:
            return user_word
        cur = diff_function(user_word, i, limit)
        if cur <= limit:
            if dif > cur:
                ok, dif = i, cur
    return ok
...
def pawssible_patches(start, goal, limit):
    ...
    def f(start, goal, limit, diff):
        if diff > limit:
            return diff
        if len(start) == 0: # need to add
            return diff + len(goal) 
        if len(goal) == 0: # need to remove
            return diff + len(start)
        if start[0] == goal[0]:
            return f(start[1:], goal[1:], limit, diff)
        return min(f(start[1:], goal, limit, diff + 1), f(start[1:], goal[1:], limit, diff + 1), f(start, goal[1:], limit, diff + 1))
    return f(start, goal, limit, 0)
...
def report_progress(typed, prompt, user_id, send):
    ...
    for i in range(min(len(typed), len(prompt))):
        if typed[i] == prompt[i]:
            ...
            break
    send({'id': user_id, 'progress': progress / len(prompt)})
...
def fastest_words(game):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(game, j, i) < t:
                t, p = time(game, j, i), j
        fastest[p].append(word_at(game, i))
    return fastest
    # END PROBLEM 10
