...
def balanced(m):
    ...
    torque1 = total_weight(end(arm1)) * length(arm1) 
    torque2 = total_weight(end(arm2)) * length(arm2)
    if torque1 != torque2:
        return False
    return balanced(end(arm1)) and balanced(end(arm2))
...
def replace_leaf(t, find_value, replace_value):
    ...
    B = [replace_leaf(b, find_value, replace_value) for b in branches(t)]
...
def check_par():
    ...
    r2 = interval(-9, -8) # Replace this line!
    return r1, r2
...
def quadratic(x, a, b, c):
    ...
    "*** YOUR CODE HERE ***"
    mid = - b / (2 * a)
    my = a * mid * mid + b * mid + c
    x1, x2 = lower_bound(x), upper_bound(x)
    y1, y2 = a * x1 * x1 + b * x1 + c, a * x2 * x2 + b * x2 + c
    if x1 >= mid or x2 <= mid:
        return interval(y1, y2)
    return interval(min(y1, y2, my), max(y1, y2, my))
