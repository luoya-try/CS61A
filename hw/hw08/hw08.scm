...
(define (derive-product expr var)
  (let ((x (first-operand expr)) (y (second-operand expr)))
      (make-sum (make-product (derive x var) y) (make-product x (derive y var)))
  )
)
...
(define (exp? exp)
  (and (list? exp) (eq? (car exp) '^) (variable? (cadr exp)) (number? (caddr exp)) (> (caddr exp) 1))
)
; f(x)^(g(x) - 1) * g(x)
...
(define (derive-exp exp var)
  (let ((f (cadr exp)) (g (caddr exp)))
        (make-product g (make-exp f (- g 1)))
  )
)
