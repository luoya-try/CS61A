...
def sprout_leaves(t, leaves):
    ...
    """
    "*** YOUR CODE HERE ***"
    leaves_ = [tree(i) for i in leaves]
    if is_leaf(t):
        return tree(label(t), leaves_)
    tmp = []
    for i in branches(t):
        tmp += [sprout_leaves(i, leaves)]
...
def add_trees(t1, t2):
    ...
    """
    逐层看 首先新树必然是树根相加
    最简单：如果一棵树没有子树了， 直接将另一棵树的子树作为新树子树
    如果两个树都有子树 两个子树从左到右匹配 继续dfs 直到后面剩下子树
    """
    b = []
    ...
    if len(branches(t1)) == 0:
        ...
        for i in range(len(branches(t1))):
            b.append(add_trees(branches(t1)[i], branches(t2)[i]))
        b += branches(t2)[len(branches(t1)):]
