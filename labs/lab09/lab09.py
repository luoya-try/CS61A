...
def inc_subseqs(s):
    ...
    """
    #s的单调的序列已经求出，可以看作存放在f(s[1:], s[0])中
    #现在求解选取s[0]或不选取s[0]两种类型的上升子序列
    def ok(s):
        lst = 0
        for i in s:
            if i < lst:
...
def trade(first, second):
    ...
    while m <= len(first) and n <= len(second) and (sum(first[:m]) != sum(second[:n]) or (n == 1 and m == 1)):
